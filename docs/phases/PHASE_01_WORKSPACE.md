...
## Purpose
Validate the host toolchain and stop at the gate.
...
5. Run `scripts/github/create-phase-checkpoint.sh PHASE-01 "Workspace" origin` to create the commit, immutable tag, remote push, and snapshot.
9. Run `scripts/github/verify-phase-closeout.sh PHASE-01 origin` on Linux to verify tag identity, commit reachability, snapshot SHA-256, and recorded evidence.
