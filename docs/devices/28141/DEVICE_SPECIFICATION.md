# Waveshare ESP32-S3-Touch-LCD-4.3B-BOX

SKU: 28141

Official exact-SKU sources of truth:
- https://www.waveshare.com/esp32-s3-touch-lcd-4.3b.htm?sku=28141
- https://docs.waveshare.com/ESP32-S3-Touch-LCD-4.3B

The 4.3B box version is treated as the same electronics family as the non-box 4.3B variant unless an exact source states otherwise.

All hardware facts are initially UNVERIFIED. Populate only from approved exact-SKU sources.
