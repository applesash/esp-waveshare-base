...
Phase-02 safe-state baseline: source-backed initialization levels for LCD reset, touch reset, backlight, and isolated DO0/DO1 are recorded in `board_safe_states.h` and `GPIO_REGISTER.md`. Owner-approved policy leaves external isolated outputs unconfigured until an authorized I/O service owns them. The CH422G raw endpoints are resolved by the datasheet: command byte `0x48` uses endpoint `0x24`, and command byte `0x70` uses endpoint `0x38`; the high-level `0x20` notation does not replace command-specific endpoint access.
